...
wp_enqueue_script( 'gcc-wp-2018-theme-scripts', get_template_directory_uri() . '/dist/scripts/theme.min.js', array(), '201831', true );
...
function add_defer_attribute($tag, $handle) {
   ...
   $scripts_to_defer = array('gcc-wp-2018-theme-scripts');
...
function gcc_wp_2018_move_jquery_to_footer() {
 	// unregister the jQuery at first
        wp_deregister_script('jquery');

        // register to footer (the last function argument should be true)
        wp_register_script('jquery', get_theme_file_uri('/dist/scripts/jquery.min.js'), false, null, false);
